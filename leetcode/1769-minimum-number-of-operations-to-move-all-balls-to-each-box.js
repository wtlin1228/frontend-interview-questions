 ...
 * box j if abs(i - j) == 1. Note that after doing so, there may be more than one ball in some boxes.
 *
...
/**
 * Time complexity: O(n) where n = boxes.length
 * Space complexity: O(n) where n = boxes.length
 * @param {string} boxes
 * @return {number[]}
 */
