...
function boundaryOfBinaryTree(root) {
  ...
  if (!root.left) {
    leftBoundaries.push(root.val)
  } else {
    node = root
    while (node) {
      leftBoundaries.push(node.val)
  ...
  while (currentLayer.length !== 0) {
    ...
    for (let node of currentLayer) {
      ...
      !node.left && !node.right && leaves.push(node.val)
    }
    currentLayer = nextLayer
  }

  return [...leftBoundaries, ...leaves, ...rightBoundaries.reverse()].reduce(
    (acc, curr) => {
...
console.log(
  boundaryOfBinaryTree(new Node(1, null, new Node(2, new Node(3), new Node(4))))
)
...
console.log(
  boundaryOfBinaryTree(
    new Node(
      ...
      new Node(3, new Node(6, new Node(9), new Node(10)))
    )
  )
)
